...
window.onload = function () {
    ...
    //load images
    // birdImg = new Image();
    // birdImg.src = "./flappy-bird-gif.png";
...
function update() {
    ...
    velocityY += gravity;
    // bird.y += velocityY;
    bird.y = Math.max(bird.y + velocityY, 0);//apply gravity to current bird.y, limit the body.y to top of the canvas 
    // context.drawImage(birdImg, bird.x, bird.y, bird.width, bird.height);
    ...
    for (let i = 0; i < pipeArray.length; i++) {
        ...
        if (!pipe.passed && bird.x > pipe.x + pipe.width) {
            score += 0.5;//0.5 because there is two pipe! that's why 0.5
            pipe.passed = true;
        }
...
function placePipes() {
    ...
    //(0-1) * pipeHEight/2.
    //0 -> -128 (pipeHeight/4)
    // 1-> -128 -256 (ipeHeight/4 - pipeHeight/2) = -3/4 pipeHeight
